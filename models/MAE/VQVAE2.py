    ...
    def forward(self, x):
        ...
        for l in range(self.nb_levels-1, -1, -1):
            ...
            if len(decoder_outputs): # if we have previous levels to condition on
                code_q, code_d, code_i = codebook(torch.cat([encoder_outputs[l], decoder_outputs[-1]], axis=1))
            else:
                code_q, code_d, code_i = codebook(encoder_outputs[l])
            diffs.append(code_d)
            ...
            code_ids.append(code_i)
            upscale_counts.append(0)

        return decoder_outputs[-1], diffs, encoder_outputs, code_outputs, code_ids, decoder_outputs
